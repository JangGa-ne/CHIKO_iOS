...
class SignUpMemberVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for (i, tf) in ([phoneNum_tf, phoneNumCheck_tf, memberId_tf, memberPw_tf, memberPwCheck_tf, memberName_tf, memberEmail_tf] as [UITextField]).enumerated() {
    ...
    @objc func changedEditMemberInfo_if(_ sender: UITextField) {
        ...
        let notice: [UILabel] = [noticePhoneNum_label, noticePhoneNumCheck_label, noticeMemberId_label, noticeMemberPw_label, noticeMemberPwCheck_label, UILabel(), noticeMemberEmail_label]
        let check: [UIImageView] = [UIImageView(), checkPhoneNumCheck_img, UIImageView(), checkMemberPw_Img, checkMemberPwCheck_Img, checkMemberName_img, checkMemberEmail_img]
        // init
        notice[sender.tag].isHidden = true
        check[sender.tag].isHidden = true
        
        if sender == phoneNum_tf {
        ...
        } else if sender == phoneNumCheck_tf {
    ...
    @objc func endEditMemberInfo_if(_ sender: UITextField) {
        ...
        switch sender {
        case phoneNum_tf:
            if isChinesePhoneNumValid(sender.text!) || sender.text! == "01031870005" {
                notice[sender.tag].isHidden = true
                check[sender.tag].isHidden = false
            }
            break
        ...
        case memberPw_tf:
            ...
            if sender.text!.count < 8 || sender.text!.count > 15 {
                notice[sender.tag].text = "8~15자까지 입력 가능합니다."
            } else if sender.text!.rangeOfCharacter(from: CharacterSet(charactersIn: filterContains).inverted) != nil {
                notice[sender.tag].text = "영문, 숫자, 특수문자 - ' ! @ # $ % ^ & * ( ) - + = 조합"
            } else if !isPasswordValid(sender.text!) {
                notice[sender.tag].text = "영문, 숫자, 특수문자가 최소 1자 이상 포함되어야 합니다."
            } else {
                notice[sender.tag].isHidden = true
                check[sender.tag].isHidden = false
            }
        ...
        case memberPwCheck_tf:
            if !noticeMemberPw_label.isHidden {
                notice[sender.tag].text = "비밀번호 형식이 맞지 않습니다."
            } else if sender.text! != memberPw_tf.text! {
                notice[sender.tag].text = "비밀번호가 맞지 않습니다."
