...
class WhGoodsUploadCC: UICollectionViewCell {
    ...
    @objc func end_optionPrice_tf(_ sender: UITextField) {
        ...
        if WhGoodsUploadVCdelegate.GoodsObject.item_sale_price > price {
            ...
            WhGoodsUploadVCdelegate.OptionPriceArray[indexpath_row].size_price[sender.tag].price = WhGoodsUploadVCdelegate.GoodsObject.item_sale_price
        } else {
...
class WhGoodsUploadTC: UITableViewCell {
    ...
    @objc func edit_textfield(_ sender: UITextField) {
        ...
        if sender.tag == 0 {
            WhGoodsUploadVCdelegate.GoodsObject.item_name = sender.text!
        } else if sender.tag == 1 {
    ...
    @objc func end_textfield(_ sender: UITextField) {
        ...
        if sender == itemPrice_tf {
            ...
            if 100 > item_sale_price {
                WhGoodsUploadVCdelegate.notice_sale_price = false; noticeItemSalePrice_label.text = "할인가를 정확히 입력해 주세요.(최소 단위: 100)"
    ...
    @objc func select_btn(_ sender: UIButton) {
        ...
        if sender.tag == 0 {
            ...
            if WhGoodsUploadVCdelegate.GoodsObject.item_sale_price == 0 {
    ...
    @objc func materialWashing_btn(_ sender: UIButton) {
        ...
        materialWashing_btns.forEach { btn in
            ...
            case (0...2, true):
                ...
                WhGoodsUploadVCdelegate.GoodsObject.item_material_washing["thickness"] = material_washing
            case (3...5, true):
                WhGoodsUploadVCdelegate.MaterialInfoArray["see_through"] = material_washing
                WhGoodsUploadVCdelegate.GoodsObject.item_material_washing["see_through"] = material_washing
            case (6...8, true):
                WhGoodsUploadVCdelegate.MaterialInfoArray["flexibility"] = material_washing
                WhGoodsUploadVCdelegate.GoodsObject.item_material_washing["flexibility"] = material_washing
            case (9...11, true):
    ...
    @objc func otherType_btn(_ sender: UIButton) {
        ...
        otherType_btns.forEach { btn in
            ...
            case (0...1, true):
                ...
                WhGoodsUploadVCdelegate.GoodsObject.item_build = other_type
            case (2...3, true):
                WhGoodsUploadVCdelegate.OtherTypeArray["manufacture_country"] = other_type
                WhGoodsUploadVCdelegate.GoodsObject.item_manufacture_country = other_type
            case (4...5, true):
                WhGoodsUploadVCdelegate.OtherTypeArray["disclosure"] = other_type
                WhGoodsUploadVCdelegate.GoodsObject.item_disclosure = other_type
            default:
                break
            }
        }
...
extension WhGoodsUploadTC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        } else if collectionView == colorCollectionView, WhGoodsUploadVCdelegate.GoodsObject.item_colors.count > 0 {
            return WhGoodsUploadVCdelegate.GoodsObject.item_colors.count
        ...
        } else if collectionView == materialCollectionView, WhGoodsUploadVCdelegate.GoodsObject.item_materials.count > 0 {
            return WhGoodsUploadVCdelegate.GoodsObject.item_materials.count
        } else {
            return .zero
        }
    }
...
//            UIView.setAnimationsEnabled(false); WhGoodsUploadVCdelegate.tableView.reloadSections(IndexSet(integer: 2), with: .none); UIView.setAnimationsEnabled(true)
            ...
            WhGoodsUploadVCdelegate.GoodsObject.item_style = WhGoodsUploadVCdelegate.StyleArray[indexPath.row]
        } else if collectionView == contentCollectionView {
            ...
            WhGoodsUploadVCdelegate.GoodsObject.item_materials.remove(at: indexPath.row)
            materialCollectionView.isHidden = (WhGoodsUploadVCdelegate.GoodsObject.item_materials.count == 0)
        }
        collectionView.reloadData()
    }
}
