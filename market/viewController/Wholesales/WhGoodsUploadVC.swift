...
extension WhGoodsUploadVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didEndDisplaying cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            ([cell.colorCollectionView, cell.sizeCollectionView, cell.materialCollectionView] as [UICollectionView]).enumerated().forEach { i, collectionView in
                collectionViewContentOffsets[i+1] = collectionView.contentOffset.x
            }
        } else if indexPath.section == 2 {
            collectionViewContentOffsets[4] = cell.optionPriceCollectionView.contentOffset.x
        } else if indexPath.section == 3 {
            ([cell.styleCollectionView, cell.contentCollectionView] as [UICollectionView]).enumerated().forEach { i, collectionView in
                collectionViewContentOffsets[i+4+1] = collectionView.contentOffset.x
            }
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            ([cell.itemCategory_btn, cell.color_btn, cell.material_btn] as [UIButton]).enumerated().forEach { i, btn in
            ...
            cell.itemName_tf.tag = 0; cell.itemName_tf.addTarget(cell, action: #selector(cell.edit_textfield(_:)), for: .editingChanged)
            ...
            cell.itemPrice_tf.tag = 1; cell.itemPrice_tf.addTarget(cell, action: #selector(cell.edit_textfield(_:)), for: .editingChanged)
            if item_sale {
            ...
            cell.itemSalePrice_tf.tag = 2; cell.itemSalePrice_tf.addTarget(cell, action: #selector(cell.edit_textfield(_:)), for: .editingChanged)
