...
class WhGoodsUploadTC: UITableViewCell {
    ...
    func viewDidLoad() {
        ...
        if indexpath_section == 0 {
            ...
            ([colorCollectionView, sizeCollectionView, materialCollectionView] as [UICollectionView]).enumerated().forEach { i, collectionView in
                
                if collectionView != sizeCollectionView { collectionView.backgroundColor = .white }
                
                collectionView.delegate = nil; collectionView.dataSource = nil
                
                let layout = UICollectionViewFlowLayout()
                if collectionView == colorCollectionView || collectionView == materialCollectionView {
            ...
            optionPriceCollectionView.contentOffset.x = WhGoodsUploadVCdelegate.collectionViewContentOffsets[4] ?? 0.0
            optionPriceCollectionView.delegate = self; optionPriceCollectionView.dataSource = self
        } else if indexpath_section == 3 {
            
            ([styleCollectionView, contentCollectionView] as [UICollectionView]).enumerated().forEach { i, collectionView in
                ...
                layout.sectionInset = UIEdgeInsets(top: 0, left: 20, bottom: 0, right: 20)
                collectionView.setCollectionViewLayout(layout, animated: true, completion: nil)
                collectionView.contentOffset.x = WhGoodsUploadVCdelegate.collectionViewContentOffsets[i+4+1] ?? 0.0
                collectionView.delegate = self; collectionView.dataSource = self
            }
        }
    ...
    @objc func category_btn(_ sender: UIButton) {
        ...
        if sender.tag != 0, itemCategory_btn.titleLabel?.text ?? "" == "" {
        ...
        segue.WhGoodsUploadTCdelegate = self
...
extension WhGoodsUploadTC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if collectionView == itemCollectionView, indexPath.section == 0 {
            ...
            if itemCategory_btn.titleLabel?.text ?? "" == "" {
                WhGoodsUploadVCdelegate.customAlert(message: "카테고리를 선택해 주세요.", time: 1); return
            }
