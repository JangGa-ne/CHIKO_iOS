...
class ReBasketTC: UITableViewCell {
    ...
    @IBOutlet weak var itemTotal_view: UIView!
    @IBOutlet weak var itemTotalQuantity_label: UILabel!
    @IBOutlet weak var itemTotalPrice_label: UILabel!
    @IBOutlet weak var saleTotalPrice_label: UILabel!
...
extension ReBasketVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if data.choice {
                data.item_option.forEach { data in
                    sale_total += (BasketArray[indexPath.row].item_price-BasketArray[indexPath.row].item_sale_price)*data.quantity
                    order_total += data.price*data.quantity
                }
            }
            
            cell.itemTotal_view.layer.cornerRadius = 10
            cell.itemTotal_view.layer.maskedCorners = [.layerMinXMaxYCorner, .layerMaxXMaxYCorner]
            cell.itemTotalPrice_label.text = "\(priceFormatter.string(from: (sale_total+order_total) as NSNumber) ?? "0")원"
            cell.saleTotalPrice_label.text = "\(priceFormatter.string(from: sale_total as NSNumber) ?? "0")원"
            ...
            cell.itemTotalQuantity_label.text = "\(choice_total)개"
            cell.itemTotalPrice_label.text = "\(priceFormatter.string(from: (sale_total+order_total) as NSNumber) ?? "0")원"
            cell.saleTotalPrice_label.text = "\(priceFormatter.string(from: sale_total as NSNumber) ?? "0")원"
            cell.orderTotalPrice_label.text = "\(priceFormatter.string(from: order_total as NSNumber) ?? "0")원"
